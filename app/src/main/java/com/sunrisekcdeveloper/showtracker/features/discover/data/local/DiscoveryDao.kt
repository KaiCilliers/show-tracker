...
abstract class DiscoveryDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    abstract suspend fun insertMovieDump(vararg movie: MediaEntity.MovieEntityTMDB)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    abstract suspend fun insertShowDump(vararg movie: MediaEntity.ShowEntityTMDB)
