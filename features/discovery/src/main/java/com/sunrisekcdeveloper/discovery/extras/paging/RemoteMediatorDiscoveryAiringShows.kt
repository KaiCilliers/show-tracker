...
package com.sunrisekcdeveloper.discovery.extras.paging
...
class RemoteMediatorDiscoveryAiringShows(
...
//                    throw InvalidObjectException("Remote key should not be null for $loadType")
        ...
        // todo original had no double bang
        return when (val response = remote.airingTodayShows(page!!)) {
            is NetworkResult.Success -> {

                val uiModels =
                    response.data.media.map { it.asUIModelDiscovery(ListType.ShowAiringToday) }
                ...
                try {
                    ...
                    database.withTransaction {
                        ...
                        if (loadType == LoadType.REFRESH) {
                            ...
                            database.discoveryDao().clearList(ListType.ShowAiringToday)
                        }
