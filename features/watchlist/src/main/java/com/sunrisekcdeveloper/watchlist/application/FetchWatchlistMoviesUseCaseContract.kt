...
package com.sunrisekcdeveloper.watchlist.application
...
interface FetchWatchlistMoviesUseCaseContract {
    suspend operator fun invoke(filterOption: FilterMovies): Flow<Resource<List<UIModelWatchlistMovie>>>
    class Fake() : FetchWatchlistMoviesUseCaseContract {
        override suspend fun invoke(filterOption: FilterMovies): Flow<Resource<List<UIModelWatchlistMovie>>> {
            return flow {
                emit(Resource.loading())
                emit(Resource.success(
                    UIModelWatchlistMovie.create(45)
                ))
            }
        }
    }
